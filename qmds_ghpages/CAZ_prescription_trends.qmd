...
Loading for annxiolytics and antidepressants
...
Loading for Methylprednisolone
...
### Subsetting data of Anxiolytics andAntidepressants

```{r}
CAZ_antd <- all_antd_data[row_id %in% CAZ_practices$code, ][, `:=`(
  month = month(date),
  year = year(date)
)] |>
  left_join(
...
CAZ_anx <- all_anx_data[row_id %in% CAZ_practices$code, ][, `:=`(
  month = month(date),
  year = year(date)
)] |>
  left_join(
...
### Subsetting data of Oral steroids 

```{r}
CAZ_meth <- all_meth_data[row_id %in% CAZ_practices$code, ][, `:=`(
  month = month(date),
  year = year(date)
)] |>
  left_join(
...
bnf_codes <- plots_mtlkst$bnf |> unique()

caz_codes <- plots_mtlkst$CAZ_name |> unique()

# Loop to display title and plot
lapply(seq_along(plots_mtlkst$plots), \(i) {
  title <- plots_mtlkst$bnf[i]
  caz <- plots_mtlkst$CAZ_name[i]
  ggsave(
    plot = plots_mtlkst$plots[
      plots_mtlkst$bnf == title & plots_mtlkst$CAZ_name == caz
    ],
    units = "mm",
    width = 180,
    height = 120,
...
```{r}
noCAZ_antd_aggre <- noCAZ_antd |>
  summarise(
    across(c(NUMBER_OF_PATIENTS, items, quantity), \(x) sum(x, na.rm = T)),
    .by = c(location_class, month, year, date, bnf)
  )
noCAZ_antd_aggre$date <- as.Date(noCAZ_antd_aggre$date)
CAZ_antd$date <- as.Date(CAZ_antd$date)

noCAZ_antd_nest <- noCAZ_antd_aggre |>
  nest(.by = bnf, .key = "control_data")
```
...
plots_antd <- CAZ_antd |>
  ...
  left_join(noCAZ_antd_nest, by = "bnf") |>
  mutate(
    trend_data = map2(data, Start, \(.x, .y) {
      .x |>
        filter(year >= .y)
...
bnf_codes <- plots_antd$bnf |> unique()

caz_codes <- plots_antd$CAZ_name |> unique()

# Loop to display title and plot
lapply(seq_along(plots_antd$plots), \(i) {
  title <- plots_antd$bnf[i]
  caz <- plots_antd$CAZ_name[i]
  ggsave(
    plot = plots_antd$plots[
      plots_antd$bnf == title & plots_antd$CAZ_name == caz
    ],
    units = "mm",
    width = 180,
    height = 120,
...
```{r}
noCAZ_anx_aggre <- noCAZ_anx |>
  summarise(
    across(c(NUMBER_OF_PATIENTS, items, quantity), \(x) sum(x, na.rm = T)),
    .by = c(location_class, month, year, date, bnf)
  )
noCAZ_anx_aggre$date <- as.Date(noCAZ_anx_aggre$date)
CAZ_anx$date <- as.Date(CAZ_anx$date)

noCAZ_anx_nest <- noCAZ_anx_aggre |>
  nest(.by = bnf, .key = "control_data")
```
...
plots_anx <- CAZ_anx |>
  ...
  left_join(noCAZ_anx_nest, by = "bnf") |>
  mutate(
    trend_data = map2(data, Start, \(.x, .y) {
      .x |>
        filter(year >= .y)
...
bnf_codes <- plots_anx$bnf |> unique()

caz_codes <- plots_anx$CAZ_name |> unique()

# Loop to display title and plot
lapply(seq_along(plots_anx$plots), \(i) {
  title <- plots_anx$bnf[i]
  caz <- plots_anx$CAZ_name[i]
  ggsave(
    plot = plots_anx$plots[
      plots_anx$bnf == title & plots_anx$CAZ_name == caz
    ],
    units = "mm",
    width = 180,
    height = 120,
...
```{r}
noCAZ_meth_aggre <- noCAZ_meth |> 
  summarise(
    across(c(NUMBER_OF_PATIENTS, items, quantity), \(x) sum(x, na.rm = T)),
    .by = c(location_class, month, year, date, bnf)
  )

noCAZ_meth_aggre$date <- as.Date(noCAZ_meth_aggre$date)
CAZ_meth$date <- as.Date(CAZ_meth$date)

noCAZ_meth_nest <- noCAZ_meth_aggre |> 
  nest(.by = bnf, .key = "control_data")
```
...
plots_meth <- CAZ_meth |>
  ...
  left_join(noCAZ_meth_nest, by = "bnf") |>
  mutate(
    trend_data = map2(data, Start, \(.x, .y) {
      .x |>
        filter(year >= .y)
...
bnf_codes <- plots_meth$bnf |> unique()

caz_codes <- plots_meth$CAZ_name |> unique()

# Loop to display title and plot
lapply(seq_along(plots_meth$plots), \(i) {
  title <- plots_meth$bnf[i]
  caz <- plots_meth$CAZ_name[i]
  ggsave(
    plot = plots_meth$plots[
      plots_meth$bnf == title & plots_meth$CAZ_name == caz
    ],
    units = "mm",
    width = 180,
    height = 120,
...
# Loop to display title and plot
lapply(seq_along(plots_bnf$plots), \(i) {
  title <- plots_bnf$bnf[i]
  caz <- plots_bnf$CAZ_name[i]
  ggsave(
    plot = plots_bnf$plots[
      plots_bnf$bnf == title & plots_bnf$CAZ_name == caz
    ],
    units = "mm",
    width = 180,
    height = 120,
