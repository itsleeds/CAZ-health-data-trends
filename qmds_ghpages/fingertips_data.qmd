...
Obtaining indicators for broader cardiovascular disease using profile code 135.

```{r}
indicator_lst_cardio <- indicators(ProfileID = 135)

indicator_lst_cardio |>
    head(10) |>
    knitr::kable()
```
...
sel_indicator <- c(90810, 90986, 212)
```
...
Downloading actual data for the indicators Lower and Upper tier LAs.

```{r}
#| label: get_fingertips_data
#| output: false
temp_data <- fingertips_data(
    IndicatorID = sel_indicator,
    AreaTypeID = "All"
)
...
```{r}
write_csv(temp_data, "data_raw/fingertips_data.csv", append = F)
```
