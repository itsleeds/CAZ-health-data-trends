...
admissions_control <- admissions_data |>
  ...
  filter(Sex == "Persons") |>
  summarise(
    across(c(Count, Denominator), \(x) sum(x, na.rm = T)),
    .by = c(TimeperiodSortable, Age)
  ) |>
  mutate(Value = (1e5 * Count) / Denominator, AreaName = "Control")
```
...
admissions_plot <- CAZ_admissions |>
  ...
  geom_line(data = admissions_control, col = "black", linetype = "dashed") +
  geom_line() +
...
CAZ_admissions |>
  ...
  kable_paper() |>
  scroll_box(width = "100%", height = "5%")
```

Producing individual plots for each CAZ

```{r}
names_correspondence <- bind_cols(
  caz_boundaries |>
    st_drop_geometry() |>
    arrange(name) |>
    select(name),
...
for (i in 1:nrow(names_correspondence)) {
  caz_name <- names_correspondence$name[i]
  area_name <- names_correspondence$AreaName[i]

  admissions_tmp <- CAZ_admissions |>
    filter(AreaName == area_name, Sex == "Persons") |>
    ggplot(aes(x = TimeperiodSortable, y = Value, col = AreaName)) +
    geom_line(data = admissions_control, col = "black", linetype = "dashed") +
    geom_line() +
    scale_x_continuous(breaks = unique(CAZ_admissions$TimeperiodSortable)) +
