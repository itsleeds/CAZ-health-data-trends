...
if (!file.exists(file.path(here(),"data_raw","practice_patients_age.csv"))){
  ...
  for (i in my_grid$u) {
    ...
    my_link <- links[grep(links, pattern = "gp-reg-pat-prac-sing-age-(fe)?male")]

    for (tlink in my_link){
      download.file(
        url = tlink,
        destfile = paste0(folder_path, "/", i, ".", tools::file_ext(tlink)),
        mode = "wb"
        )
  ...
  all_data_df_age <- read_csv(file.path(here(),"data_raw","practice_patients_age.csv"),
                          col_types = cols(
  PUBLICATION = col_character(),
  EXTRACT_DATE = col_character(),
  TYPE = col_character(),
