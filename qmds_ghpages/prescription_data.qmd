...
dates <- seq(as.Date("2021-01-01"), as.Date("2025-11-01"), "months")
...
```{r}
mont_data_grid <- expand_grid(code = c("0403","040102"), date = dates) |>
  mutate(group = nchar(code), file.name = paste0(code, "_", date, ".csv"))
...
dir.create("data_raw/bnf_all", showWarnings = F)
all_bnf_codes <- lapply(seq_len(meth_data_grid |> nrow()), \(x) {
  if (
    !file.exists(file.path("data_raw", "bnf_all", meth_data_grid$file.name[x]))
  ) {
    download.file(
      paste0(
        "https://openprescribing.net/api/1.0/spending_by_org/?org_type=practice&code=",
        meth_data_grid$code[x],
        "&date=",
        meth_data_grid$date[x],
        "&format=csv"
      ),
      destfile = file.path("data_raw", "bnf_all", meth_data_grid$file.name[x]),
      mode = "wb"
    )
  }
})
